...
from .serializers import EmergencyCategorySerializer, IncidentUpdateCreateSerializer, IncidentUpdateSerializer, SOSCreateSerializer, SOSSerializer
...
class SOSListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            print(f"Notification/Escalation Error: {e}")
...
class CommunityBroadcastView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        volunteers = User.objects.filter(role="volunteer", is_verified=True)
        security = User.objects.filter(role="security", is_verified=True)
        recipients = list(volunteers) + list(security)
        notified = 0
        for recipient in recipients:
            try:
                from notifications.services import NotificationService
                NotificationService.send_push_notification(recipient, "Community Broadcast SOS", f"SOS from {sos.resident.email} within {radius_km}km")
                notified += 1
            except Exception:
                pass
        return Response({"notified": notified, "recipients": len(recipients)}, status=status.HTTP_200_OK)
...
class VolunteerAvailabilityView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.role != "volunteer":
            return Response({"detail": "Only volunteers can access this"}, status=status.HTTP_403_FORBIDDEN)
        return Response({"user_id": user.id, "available": True, "role": user.role}, status=status.HTTP_200_OK)
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"user_id": user.id, "available": available, "role": user.role}, status=status.HTTP_200_OK)
